...
> - 画像は `images/` に配置する。サイズは `{width=70%}` または `<figure class="doc-figure img-width-md">` で指定する（詳細は README）。
> - 図番号・表番号は `<div class="doc-figure">` と `<p class="doc-figure-caption">` / `<p class="table-caption">` を使う。番号は CSS で自動採番される。
...
<img src="images/作業概要イメージ.png" alt="作業概要イメージ">
</div>
<p class="doc-figure-caption">作業概要（バックアップ→更新→確認の流れ）</p>
</div>
