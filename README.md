...
**重要:** 図番号・表番号・中央配置は `style.less` に定義されています。更新後は MPE プレビューを開き直してください。
...
画像は `images/` フォルダに配置します。**図番号は CSS で自動採番**されます（手動で「図1」と書く必要はありません）。

MPE では `<figure>` 内に Markdown を書くと崩れるため、**`<div class="doc-figure">` を使ってください。**

#### 画像の例

```html
<div class="doc-figure img-width-70">
<div class="doc-figure-body">
<img src="images/サンプル.png" alt="説明">
</div>
<p class="doc-figure-caption">キャプション文</p>
</div>
```
...
<p class="doc-figure-caption">作業全体フロー</p>
</div>
```

### 4. 表番号の付け方

Markdown の表の直前に `<p class="table-caption">` を置きます。**表番号も CSS で自動採番**されます。

```markdown
<p class="table-caption">固定情報（GUI等）</p>
...
- 挿絵・スクリーンショットのサイズ指定（`{width=...}` / サイズクラス）
- 図番号・表番号の自動採番（`doc-figure` / `table-caption`）
